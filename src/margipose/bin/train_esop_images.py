...
import tele
import torch
from esop.bin.train_margipose import MargiPoseTrainEngine
from esop.config import get_dataset_path
from esop.datasets.mpi3d import Mpi3dDataset
...
def _inject_batch_input_from_esop_data(batch, dataset, esop_data):
    images = []
    for index in batch['index']:
        frame_ref = dataset.frame_refs[int(index)]
        new_example = esop_data.get_specific_example(
            frame_ref.subject_id, frame_ref.sequence_id, frame_ref.camera_id,
            frame_ref.frame_index)
        images.append(
            new_example['transformed_image'].to(global_opts['device'], torch.float32))
    batch['input'] = torch.stack(images, dim=0)


def do_training_pass(epoch, model, tel, loader, esop_data, scheduler, on_progress):
    ...
    for batch in generator_timer(progress_iter(loader, 'Training'), tel['data_load_time']):
        ...
        _inject_batch_input_from_esop_data(batch, loader.dataset, esop_data)
...
def do_validation_pass(epoch, model, tel, loader, esop_data):
    vis_images = None
...
def sacred_main(_run: Run, seed, showoff, out_dir, batch_size, epochs, tags, model_desc,
    ...
    assert optim_algorithm == 'sgd_simple'
    assert train_datasets == ['mpi3d-train']
    assert val_datasets == ['mpi3d-val']
    opts = Namespace(all_examples=False, augment=use_aug, batch_size=batch_size, bg_augmentation_images=None,
                     deterministic=deterministic, device=global_opts['device'], dry=True,
                     epochs=epochs, max_lr=None, n_train_examples=train_examples, n_val_examples=val_examples, name=None,
                     optimiser='sgd_simple', reset_principal_point=False,
                     scale='root_relative_universal' if preserve_root_joint_at_univ_scale else 'camera_relative_universal',
                     seed=seed, skeleton='mpi3d_17j',
                     train_datasets=[s.replace('-', '/') for s in train_datasets],
                     val_datasets=[s.replace('-', '/') for s in val_datasets])
    engine = MargiPoseTrainEngine(opts)
    ...
    example_specs = model_metadata.get_example_specs()
    esop_train_data = Mpi3dDataset(example_specs, get_dataset_path('mpi3d'), 'train', augment=use_aug, valid_only=False)
    esop_val_data = Mpi3dDataset(example_specs, get_dataset_path('mpi3d'), 'val', augment=False, valid_only=False)
    ...
    for epoch in trange(epochs, leave=False, ascii=True):
        ...
        do_training_pass(epoch, model, tel, train_loader, esop_train_data, scheduler, on_train_progress)
        if val_loader:
            do_validation_pass(epoch, model, tel, val_loader, esop_val_data)
...
Train_Subcommand = Subcommand(name='train-esop-images', func=main,
                              help='train a model using the esop dataloader for images')
