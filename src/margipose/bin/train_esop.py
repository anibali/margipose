...
from margipose.train_helpers import progress_iter, create_showoff_notebook, \
    learning_schedule
...
def do_training_pass(epoch, model, tel, loader, scheduler, on_progress):
    ...
    for batch in generator_timer(progress_iter(loader, 'Training'), tel['data_load_time']):
        ...
        with timer(tel['data_transfer_time']):
            in_var = batch['transformed_image'].to(global_opts['device'], torch.float32)
            target_var = batch['transformed_joints_3d'].to(global_opts['device'], torch.float32)
            mask_var = batch['transformed_joint_mask'].to(global_opts['device'], torch.float32)
        ...
        # Update progress
        samples_processed += len(batch['transformed_image'])
...
def sacred_main(_run: Run, seed, showoff, out_dir, batch_size, epochs, tags, model_desc,
    ...
    example_specs = engine.model_metadata.get_example_specs()
    train_loader, val_loader = engine.create_dataloaders(example_specs)
...
Train_Subcommand = Subcommand(name='train-esop', func=main, help='train a model using the esop dataloader')
